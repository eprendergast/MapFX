...
window.addEventListener("DOMContentLoaded", () => {
  ...
  queryForm.addEventListener('submit', () => {
    ...
    API.getLatestRatesWithBase(currency).then(latestDataResponse => API.getHistoricalData(latestDataResponse, currency, historicalDate))
  })
})

function countriesHandler(countries, currentRatesResponse, historicalRatesResponse){
  resetMap();
  renderMap(countries, currentRatesResponse, historicalRatesResponse);
}
...
function generateCountryMapId(country) {
  return `map-container-map-country-${country.code}`;
}
...
function clickedOnCountry(event, country){ // CALL NEW FETCH TO FX DATA API FROM HERE
  ...
  let countryCode = event.target.id.split("-")[4]
  API.getCountryByCountryCode(countryCode).then(console.log)

  // getCurrencyFromCountryData
}

function getCurrencyFromCountryData(countryData){
  let currency = countryData[0].currency;
  let historicalDate = document.getElementById('query-form').elements.date.value;
  API.getLatestRatesWithBase(currency).then(latestDataResponse => API.getHistoricalData(latestDataResponse, currency, historicalDate))
}
